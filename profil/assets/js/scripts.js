(function ($) {
	...
	$('.contact-form').on('submit', function() {
		...
		if (validateEmail(contactForm.find('.contact-email').val()) && contactForm.find('.contact-email').val().length !== 0 && contactForm.find('.contact-name').val().length !== 0 && contactForm.find('.contact-message').val().length !== 0) {
			...
			$.ajax({
				...
				success: function() {
					contactForm.find('.contact-loading').fadeOut();
					contactForm.find('.contact-success').find('.message').html('Succès! Merci de m\'avoir contacté!');
					contactForm.find('.contact-success').fadeIn();
				},
				error: function() {
					contactForm.find('.contact-loading').fadeOut();
					contactForm.find('.contact-error').find('.message').html('Désolé, une erreur s\'est produite.');
			...
			contactForm.find('.contact-success').fadeOut();
			contactForm.find('.contact-loading').fadeOut();
			contactForm.find('.contact-error').find('.message').html('S\'il vous plaît entrez une adresse email valide.');
			contactForm.find('.contact-error').fadeIn();
		} else {
			contactForm.find('.contact-error').fadeOut();
			contactForm.find('.contact-success').fadeOut();
			contactForm.find('.contact-loading').fadeOut();
			contactForm.find('.contact-error').find('.message').html('S\'il vous plaît remplissez tous les champs');
			contactForm.find('.contact-error').fadeIn();
		}
		return false;
	});
	...
	function initialize_map() {
		if ($('.map').length) {
			var myLatLng = new google.maps.LatLng(37.4219999,-122.0840575);
			var mapOptions = {
				zoom: 15,
				center: myLatLng,
				scrollwheel: false,
